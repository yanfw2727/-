...
static void DES(char Out[8], char In[8], const PSubKey pSubKey, bool Type);//?准DES加/解密
static void SetKey(const char* Key, int len);// ?置密?
static void SetSubKey(PSubKey pSubKey, const char Key[8]);// ?置子密?
static void F_func(bool In[32], const bool Ki[48]);// f 函?
static void S_func(bool Out[32], const bool In[48]);// S 盒代替
static void Transform(bool *Out, bool *In, const char *Table, int len);// ??
static void Xor(bool *InA, const bool *InB, int len);// 异或
static void RotateL(bool *In, int len, int loop);// 循?左移
static void ByteToBit(bool *Out, const char *In, int bits);// 字????成位?
static void BitToByte(char *Out, const bool *In, int bits);// 位???成字??
...
static bool SubKey[2][16][48];// 16圈子密?
static bool Is3DES;// 3次DES?志
...
SetKey(Key, keylen);
if( !Is3DES ) { // 1次DES
for(long i=0,j=datalen>>3; iDES(Out, In, &SubKey[0], Type);
}
else{
...
Transform(M, M, IPR_Table, 64); //最后的逆置?
BitToByte(Out, M, 64);
}
...
char str[]="Welcome to 3-DES Test! --zhanglong\n"
