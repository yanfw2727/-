...
static void f_func(bool in[32],const bool ki[48]);//f函?
static void s_func(bool out[32],const bool in[48]);//s盒代替
//??
static void transform(bool *out, bool *in, const char *table, int len);
static void mxor(bool *ina, const bool *inb, int len);//异或
...
static bool subkey[16][48];//16圈子密?
...
void mxor(bool *ina,const bool *inb,int len)
{
       for(int i=0;i<len;i++)
              ina[i]^=inb[i];
}
