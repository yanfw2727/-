...
int AES_encrypt(unsigned char EnCryptDest[], const unsigned char EnCryptSrc[], unsigned int iSrcByteSize)
{
	int i;
	int iBlockIdx;	
	unsigned int iOutByteIdx = 0;
	unsigned int uiTotalBlockSize  = 0;
	int iBlockByteSize = Nk*4;
	int iBlockSize = Nk*4;

	if (iSrcByteSize ==0) return FALSE ;
	if (iSrcByteSize%iBlockByteSize !=0) return FALSE;

	uiTotalBlockSize = (iSrcByteSize-1) / (iBlockByteSize) + 1;	

	// The KeyExpansion routine must be called before encryption.
	//KeyExpansion();

	for (iBlockIdx =0; iBlockIdx < uiTotalBlockSize ; iBlockIdx++)
	{
		iBlockSize = iBlockByteSize;
		if (iBlockIdx ==uiTotalBlockSize-1 )
		{
			iBlockSize = iSrcByteSize - (iBlockIdx*iBlockByteSize);
		}	

		for(i=0;i<iBlockByteSize;i++)
		{	
			if (i < iBlockSize)
			{			
				in[i]=EnCryptSrc[iBlockIdx*iBlockByteSize+ i];
			}
			else
			{
				in[i]=0x00;
		...
		// The next function call encrypts the PlainText with the Key using AES algorithm.
		Cipher();
		memcpy(&(EnCryptDest[iOutByteIdx]),out,iBlockSize);
		iOutByteIdx =  iOutByteIdx + iBlockSize;
	}
...
int AES_decrypt(unsigned char DeCrypyDest[], const unsigned char DeCryptSrc[], unsigned int iSrcByteSize)
{
	...
	for (iBlockIdx =0; iBlockIdx < uiTotalBlockSize ; iBlockIdx++)
	{
		...
		{	
			if (i < iBlockSize)
			{			
				in[i]=DeCryptSrc[iBlockIdx*iBlockByteSize+ i];
			}
			else
			{
				in[i]=0x00;
		...
		// The next function call encrypts the PlainText with the Key using AES algorithm.
		InvCipher();
		memcpy(&(DeCrypyDest[iOutByteIdx]),out,iBlockSize);
		iOutByteIdx =  iOutByteIdx + iBlockSize;
	}
...
int AES_setkey(const unsigned char inKey[] , int nKeyBitSize)
{
	...
	if (nKeyBitSize!=128 && nKeyBitSize!=192 && nKeyBitSize!=256)
	{
		return 0;
	}
