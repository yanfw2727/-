...
//將每一個round的key和原來的矩陣作結合，然後再產生新的Key，使用xor作。 
...
//每一行都向左進行某個偏移量，而偏移量參考ShiftRowTab 
...
//expand cipher key, 並且依照128bit, 192bit, 256bit來作AES不同的Key 
...
//AES進行加密，加密順序先ADD然後四個依照下面順序作loop，最後在依序作sub, shift, add 
...
//AES進行解密，解密順序先ADD，Shift, sub然後依照下面順序作loop，最後在作一次add 
